...
function price_format_with_symbol(ct_amount){
	...
	if(currency_symbol_position == "$100"){
		return currency_symbol+ct_amount.toFixed(price_format_decimal_places);
	}else{
		return ct_amount.toFixed(price_format_decimal_places)+currency_symbol;
	}
}
/*frequently_discount*/
...
jQuery(document).on('click','.cart_frequently_discount',function(){
	...
	if(jQuery('.cart_sub_total').text() != ''){
		var cart_sub_total = jQuery('.cart_sub_total').text().split(currency_symbol);
		cart_sub_total_val = cart_sub_total[index_get_val];
		var cart_tax_val = 0;
		if(jQuery('.cart_tax').text() != ''){
			cart_tax = jQuery('.cart_tax').text().split(currency_symbol);
			cart_tax_val = cart_tax[index_get_val];
		}
		...
		jQuery('.frequent_discount').text("- "+price_format_with_symbol(freq_discount_amount));
		jQuery('.cart_total').text(price_format_with_symbol(cart_total));
