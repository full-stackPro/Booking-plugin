...
elseif(isset($_POST['confirm_booking'])){
	...
	$admin_company_name = $settingsetting->get_option('ct_company_name');
		...
		<li id="rec-noti-1" class="notificationli" data-orderid="<?php echo $b['order_id'];?>" style="background-color: <?php  echo $col;?>" data-toggle="modal" data-target="#booking-details-dashboard">
			<div class="list-inner">
				...
				if($b['client_id']==0)
				{
					...
					if($time_format == 12){
					?>
					<?php  echo str_replace($english_date_array,$selected_lang_label,date("h:i A",strtotime($b['booking_date_time'])));?></span>
					<?php 
					}else{
					?>
