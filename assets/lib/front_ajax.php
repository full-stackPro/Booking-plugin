...
else if(isset($_POST['action']) && $_POST['action']=='get_login_user_data'){
    if(!isset($_SESSION['ct_login_user_id'])){
        $u_msg=array();
        $u_msg['status']="No Login";
        echo json_encode($u_msg);die();
    }
    $user->existing_username=trim(strip_tags(mysqli_real_escape_string($conn,$_SESSION['ct_user_email'])));
